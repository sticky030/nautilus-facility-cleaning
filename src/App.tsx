...
const services = [
  {
    ...
    bullets: ['Hygienische Sanitärreinigung', 'Pflege von IT-Oberflächen', 'Aufbereitung Konferenzbereiche', 'Diskretes Abfallmanagement'],
  },
  {
    ...
    bullets: ['Glas- & Portalreinigung', 'Briefkasten- & Klingelanlagen', 'Fahrstühle & Handläufe', 'Naturstein- & Bodenpflege'],
  },
  {
    ...
    bullets: ['Baugrob- & Feinreinigung', 'Fenster- & Rahmenreinigung', 'Ersteinpflege Oberflächen', 'Bezugsfertige Endreinigung'],
  },
]
    ...
    <div className="min-h-screen bg-[#F7F4EE] text-[#6F6559] antialiased selection:bg-[#B79B6C]/20 scroll-smooth">
      ...
      <main className="overflow-x-hidden">
        ...
        <section id="start" className="relative h-screen flex items-center overflow-hidden">
          ...
          <div className="relative mx-auto max-w-7xl px-6 lg:px-10 w-full text-left">
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-10">
            ...
            <div className="grid grid-cols-1 gap-8 xl:grid-cols-3">
              {services.map((service, idx) => (
                <article key={idx} style={{ transitionDelay: `${idx * 150}ms` }} className="reveal opacity-0 translate-y-10 transition-all duration-1000 bg-white p-10 rounded-3xl border border-[#E5E1D8] shadow-sm hover:shadow-2xl transition-all transform-gpu hover:-translate-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-[#B79B6C] mb-6">{service.eyebrow}</p>
                  <h3 className="text-[26px] font-semibold text-[#2C2C2C] mb-6">{service.title}</h3>
                  <p className="text-[15px] leading-7 text-[#7E7367] mb-10">{service.text}</p>
                  <ul className="space-y-4 border-t pt-8 text-sm italic">
                    {service.bullets.map((b, j) => (
                      <li key={j} className="flex items-center gap-3"><span className="h-1.5 w-1.5 rounded-full bg-[#B79B6C]" />{b}</li>
                    ))}
                  </ul>
                </article>
              ))}
        ...
        {/* HALTUNG & TEAM (RESTORED TEXTS) */}
        <section className="bg-white py-32 lg:py-48 relative overflow-hidden text-left border-y border-[#E5E1D8]/60">
          <div className="mx-auto max-w-7xl px-6 lg:px-10 relative z-10 grid lg:grid-cols-2 gap-20 items-center">
            <div className="reveal opacity-0 translate-y-10 transition-all duration-1000">
              ...
              <p className="text-[#7E7367] leading-relaxed mb-6">Wir verstehen uns als unsichtbaren, aber essenziellen Teil Ihres Unternehmenserfolgs. Ein tadellos gepflegtes Erscheinungsbild unserer Mitarbeiter und absolute Verschwiegenheit sind für uns obligatorisch.</p>
            ...
            <div className="reveal opacity-0 translate-y-10 transition-all duration-1000 rounded-3xl overflow-hidden shadow-2xl transform-gpu">
              <img src="/images/nautilus-cleaning-team-berlin.jpg" alt="Team" className="w-full object-cover" />
            </div>
          </div>
        </section>

        {/* ABLAUF (BALANCED LAYOUT) */}
        <section id="ablauf" className="bg-[#F7F4EE] py-32 lg:py-40 text-left">
          <div className="mx-auto max-w-7xl px-6 lg:px-10 grid lg:grid-cols-12 gap-16 items-start">
            <div className="reveal opacity-0 translate-y-10 transition-all duration-1000 lg:col-span-5 lg:sticky lg:top-40">
              ...
              <p className="mt-10 text-[18px] leading-8 text-[#8A7E70] font-light">Wir überlassen Perfektion nicht dem Zufall, sondern definieren jeden Schritt für eine lautlose Integration in Ihren Alltag.</p>
            </div>
            <div className="lg:col-span-7 space-y-6">
              {[
                { s: '01 · Bedarfsanalyse', t: 'Wir analysieren Ihr Objekt vor Ort, um ein präzises Verständnis für Architektur und Diskretionsvorgaben zu entwickeln.' },
                { s: '02 · Konzeption', t: 'Maßarbeit statt Standard. Wir entwerfen ein hybrides Reinigungskonzept für Ihren individuellen Anspruch.' },
                { s: '03 · Implementierung', t: 'Lautlose Integration in Ihren Alltag. Unser Stammpersonal wird objektspezifisch instruiert.' },
                { s: '04 · Monitoring', t: 'Beständigkeit auf höchstem Niveau durch proaktives Management und regelmäßige Qualitätskontrollen.' }
              ].map((item, idx) => (
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-10 grid lg:grid-cols-2 gap-20">
            <div>
              ...
              <div className="mt-14 space-y-6 text-left">
                {[
                  { t: 'Zielgruppen', c: 'Arztpraxen, Kanzleien, Hausverwaltungen und anspruchsvolle Gewerbeeinheiten.' },
                  { t: 'Qualität', c: 'Feste Teams, geschultes Stammpersonal und absolute Zuverlässigkeit.' },
      ...
      <footer className="bg-white py-16 border-t border-[#E5E1D8] text-left">
        <div className="mx-auto flex max-w-7xl flex-col gap-8 px-6 lg:flex-row lg:items-start lg:justify-between lg:px-10">
          <div>
            ...
            <p className="mt-5 max-w-md text-[14px] leading-7 text-[#8A7E70]">Exzellenz in der Reinigung für Berlin. Mitte, Pankow, Lichtenberg, Marzahn sowie Friedrichshain-Kreuzberg.</p>
          </div>
          <div className="flex gap-8 text-[14px] font-bold uppercase tracking-wider text-[#8A7E70]">
