    ...
    <div className="min-h-screen bg-[#F7F4EE] text-[#6F6559] antialiased selection:bg-[#B79B6C]/20 scroll-smooth">
      <div className="fixed inset-x-0 top-6 z-50 flex justify-center px-4 pointer-events-none text-left">
        <header className="pointer-events-auto w-full max-w-6xl rounded-full bg-white/85 backdrop-blur-xl border border-white/60 shadow-lg px-4 py-3">
          <div className="flex items-center justify-between gap-6 px-2">
            ...
            <a href="#kontakt" className="inline-flex shrink-0 items-center rounded-full bg-[#B79B6C] px-7 py-3 text-[13px] font-bold uppercase tracking-wider text-white shadow-md transition-all hover:bg-[#A98E60]">Anfrage stellen</a>
          </div>
        </header>
      </div>
      ...
      <main className="overflow-x-hidden">
        <section id="start" className="relative h-screen flex items-center overflow-hidden bg-[#F3EFE7]">
          <video autoPlay muted loop playsInline className="absolute inset-0 h-full w-full object-cover"><source src="/videos/hero.mp4" type="video/mp4" /></video>
          ...
          <div className="relative mx-auto grid w-full max-w-7xl px-6 lg:px-10 text-left">
            <div className="reveal lg:max-w-3xl">
              ...
              <p className="mt-8 max-w-[38rem] text-[17px] leading-8 text-[#7E7367] lg:text-[19px] font-light italic">Wir betreuen Arztpraxen, Kanzleien und exklusive Gewerbeflächen in Berlin. Präzise Abstimmung, absolute Diskretion und ein Qualitätsanspruch, der im Hintergrund perfekt funktioniert.</p>
        ...
        <section id="leistungen" className="bg-[#F7F4EE] py-32 lg:py-40 text-left">
          <div className="mx-auto max-w-7xl px-6 lg:px-10">
            ...
            <div className="grid grid-cols-1 gap-8 xl:grid-cols-3">
              {services.map((s, idx) => (
                <article key={idx} className="reveal bg-white p-10 rounded-3xl border border-[#E5E1D8] shadow-sm flex flex-col h-full">
                  ...
                  <ul className="space-y-4 border-t pt-8 text-[15px] leading-6 text-[#6F6559]">
                    {s.bullets.map((b, j) => (<li key={j} className="flex items-start gap-4 italic"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#B79B6C]" />{b}</li>))}
                  </ul>
                </article>
              ))}
            </div>
        ...
        <section id="warum-nautilus" className="bg-white py-32 lg:py-40 text-left border-y border-[#E5E1D8]/60">
          <div className="mx-auto max-w-7xl px-6 lg:px-10 grid lg:grid-cols-12 gap-16">
            ...
            <div className="lg:col-span-7 grid sm:grid-cols-2 gap-6">
              {[
                { t: 'Diskrete Ausführung', d: 'Lautlose Integration in Ihr Tagesgeschäft. Unsere Teams agieren unsichtbar – das Ergebnis ist unübersehbar.' },
                { t: 'Klare Abstimmung', d: 'Ein fester, persönlicher Ansprechpartner. Proaktive Kommunikation und vorausschauendes Handeln.' },
                { t: 'Verlässliche Standards', d: 'Durch internes Qualitätsmanagement sichern wir einen konstanten, kompromisslosen Standard.' },
                { t: 'Sensible Umfelder', d: 'Speziell geschultes, verschwiegenes Personal für Bereiche, in denen Vertrauen absolute Priorität hat.' }
              ].map((item, idx) => (
                <div key={idx} className="reveal p-10 rounded-3xl border border-[#E5E1D8] bg-[#FCFBF8] shadow-sm"><h4 className="text-[10px] font-bold uppercase tracking-[0.35em] text-[#B79B6C] mb-4">{item.t}</h4><p className="text-[15px] leading-8 text-[#7E7367]">{item.d}</p></div>
              ))}
            </div>
          </div>
        </section>

        <section className="bg-[#F7F4EE] py-32 lg:py-48 text-left border-b border-[#E5E1D8]/60 text-[#6F6559]">
          <div className="mx-auto max-w-7xl px-6 lg:px-10 grid lg:grid-cols-2 gap-20 items-center">
            <div className="reveal">
              ...
              <p className="text-[#8A7E70] leading-8 italic font-light">Gerade in Notariaten, Privatpraxen und Vorstandsetagen zählt nicht die reine Anwesenheit des Personals – sondern die Fähigkeit, einen Raum mit höchster Präzision in seinen besten Zustand zu versetzen.</p>
            </div>
            <div className="reveal rounded-3xl overflow-hidden shadow-2xl"><img src="/images/nautilus-cleaning-team-berlin.jpg" alt="Team" className="w-full object-cover" /></div>
          </div>
        </section>

        <section id="ablauf" className="bg-white py-32 lg:py-40 text-left">
          <div className="mx-auto max-w-7xl px-6 lg:px-10 grid lg:grid-cols-12 gap-16 items-start">
            <div className="reveal lg:col-span-5 lg:sticky lg:top-40">
              ...
              <p className="mt-10 text-[18px] leading-8 text-[#8A7E70] font-light">Ein bewährter Prozess garantiert Ergebnisse auf höchstem Niveau. Wir überlassen nichts dem Zufall.</p>
            </div>
            <div className="lg:col-span-7 space-y-6">
              {[
                { s: '01 · Fundierte Bedarfsanalyse', t: 'Wir analysieren Ihr Objekt vor Ort, um ein präzises Verständnis für Architektur, Materialien und Ihre individuellen Diskretionsvorgaben zu entwickeln.' },
                { s: '02 · Strategische Konzeption', t: 'Maßarbeit statt Standard. Wir entwerfen ein hybrides Reinigungskonzept, das operative Effizienz mit Ihrem Anspruch an ein repräsentatives Umfeld vereint.' },
                { s: '03 · Präzise Implementierung', t: 'Lautlose Integration in Ihren Alltag. Unser festes Stammpersonal wird objektspezifisch instruiert und fügt sich ohne Reibungsverluste ein.' },
                { s: '04 · Kontinuierliches Management', t: 'Beständigkeit auf höchstem Niveau. Durch proaktives Management und regelmäßige Qualitätskontrollen stellen wir sicher, dass unser Premium-Standard dauerhaft erfüllt wird.' }
              ].map((item, idx) => (
                <div key={idx} className="reveal rounded-3xl border border-[#E5E1D8] bg-[#FCFBF8] p-10 shadow-sm"><div className="text-[11px] font-bold uppercase tracking-[0.35em] text-[#B79B6C]">{item.s}</div><p className="mt-5 text-[16px] leading-8 text-[#7E7367]">{item.t}</p></div>
              ))}
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-10 grid lg:grid-cols-2 gap-20">
            <div>
              ...
              <div className="mt-14 space-y-6">
                {[
                  { t: 'Für wen wir arbeiten', c: 'Arztpraxen, Kanzleien, Hausverwaltungen und anspruchsvolle Gewerbeimmobilien in Berlin.' },
                  { t: 'Unser Versprechen', c: 'Transparente Angebote, feste Ansprechpartner und absolute Zuverlässigkeit ab dem ersten Tag.' },
                  { t: 'Der nächste Schritt', c: 'Sie beschreiben kurz Ihr Objekt. Wir melden uns mit einer ersten Einschätzung und einem Konzept.' }
                ].map((box, idx) => (
                  <div key={idx} className="rounded-3xl border border-[#E5E1D8] bg-white p-8 shadow-sm"><p className="text-[10px] font-bold uppercase tracking-[0.35em] text-[#B79B6C] mb-2">{box.t}</p><p className="text-[#6F6559] text-[16px] leading-8">{box.c}</p></div>
                ))}
              </div>
            </div>
            <div className="reveal rounded-[40px] border border-[#E5E1D8] bg-white p-10 shadow-2xl lg:p-14">
              <form action="https://formspree.io/f/mnjonren" method="POST" className="space-y-8">
                ...
                <textarea rows={4} name="Nachricht" required placeholder="Objektbeschreibung..." className="w-full rounded-2xl border border-[#E5E1D8] bg-[#FCFBF8] px-6 py-4 outline-none focus:border-[#B79B6C] resize-none" />
      ...
      <footer className="bg-white py-16 border-t border-[#E5E1D8] text-left">
        <div className="mx-auto flex max-w-7xl flex-col gap-8 px-6 lg:flex-row lg:items-start lg:justify-between lg:px-10">
          <div><div className="text-[11px] font-bold text-[#B79B6C] tracking-[0.35em]">NAUTILUS FACILITY CLEANING</div><p className="mt-5 max-w-md text-[14px] leading-7 text-[#8A7E70]">Exzellenz in der Reinigung für Berlin Mitte, Prenzlauer Berg, Friedrichshain, Lichtenberg und Marzahn.</p></div>
          <div className="flex gap-8 text-[14px] font-bold text-[#8A7E70] uppercase"><a href="impressum/">Impressum</a><a href="datenschutz/">Datenschutz</a></div>
        </div>
      </footer>
    </div>
  )
