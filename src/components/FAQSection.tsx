    ...
    <section id="faq" className="bg-[#F7F4EE] py-32 lg:py-40 border-b border-[#E5E1D8]/60">
      <div className="mx-auto max-w-5xl px-6 lg:px-10">
        ...
        <div className="space-y-3">
          {faqs.map((faq, idx) => (
            <div key={idx} className="reveal bg-white border border-[#E5E1D8] rounded-xl shadow-sm transition-all duration-500 hover:border-[#B79B6C]/50 hover:shadow-[0_10px_30px_rgba(183,155,108,0.08)] hover:-translate-y-1">
              <button 
                onClick={() => setOpenIndex(openIndex === idx ? null : idx)} 
                className="flex w-full items-center justify-between p-6 sm:p-8 text-left group"
              >
                <span className="text-[16px] sm:text-[17px] font-semibold text-[#2C2C2C] group-hover:text-[#B79B6C] transition-colors pr-6">
                  {faq.question}
                </span>
              ...
              <div className={`transition-all duration-500 ease-in-out overflow-hidden ${openIndex === idx ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="px-6 sm:px-8 pb-8 text-[15px] sm:text-[16px] leading-8 text-[#7E7367] border-t border-[#E5E1D8]/50 pt-6 text-left whitespace-pre-line">
                  {faq.answer}
                </div>
              </div>
            </div>
